...
+ $\mathbb{E}\hat{Y}(x) = \mathbf{m}(x)^\top\beta,\ \mathrm{Cov}\left\lbrace\hat{Y}(x), \hat{Y}(x')\right\rbrace = \left(\mathbf{m}(x)^\top - \mathbf{k}(x)^\top\Sigma^{-1}\mathbf{M}\right)\left(\mathbf{M}^\top\Sigma^{-1}\mathbf{M}\right)^{-1}\left(\mathbf{m}(x) - \mathbf{M}^\top\Sigma^{-1}\mathbf{k}(x)\right),\ x,x'\in\mathcal{X}.$

## Ordinary Kriging
In ordinary kriging, the spatial field is supposed to be stationary. 
