...
<p>
  $$\hat{Y}(x) = \hat{\beta} + \mathbf{k}(x)^\top\Sigma^{-1}(\mathbf{Y} - \beta\mathbf{1}_n),\ \textsf{where}\ \ \hat{\beta} = \frac{\mathbf{1}_n^\top \Sigma\mathbf{Y}}{\mathbf{1}_n^\top \Sigma\mathbf{1}_n}.$$
</p>
