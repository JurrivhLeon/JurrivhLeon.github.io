...
Assume that $f\sim\mathcal{GP}\left(m^\top(\cdot)\beta,\ k(\cdot,\cdot)\right),$ where $beta\in\mathbb{R}^p$ is unknown, and that our observations $\left.Y(x)\right|f \sim N(f(x),\sigma^2).$ Note this assumptions implies the independence of $\left\lbrace \left.Y(x)\right|x\in\mathcal{X}\right\rbrace$ given $f.$ Then, we can define the error process
