...
<p>
  ...
  \left(\mathbf{k}(x),\mathbf{k}(x')\right)^\top\mathbf{K}^{-1}\mathbf{f},\ 
  \begin{pmatrix}
  k(x,x) & k(x,x') \\
  k(x', x) & k(x',x')
  \end{pmatrix} - \left(\mathbf{k}(x),\mathbf{k}(x')\right)^\top\mathbf{K}^{-1}\left(\mathbf{k}(x),\mathbf{k}(x')\right)\right\rbrace.$$
</p>
