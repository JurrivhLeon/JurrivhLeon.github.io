...
### Universal Kriging
In universal kriging, the mean of spatial field is unknown. To apply regression, we choose a basis $m_1(\cdot),\cdots,m_p(\cdot)$ from $L_2(\mathcal{X})$ and denote $m(\cdot) = \left(m_1(\cdot),\cdots,m_p(\cdot)\right)^\top:\mathcal{X}\to\mathbb{R}^p.$

Assume that $f\sim\mathcal{GP}\left(m^\top(\cdot)\beta,\ k(\cdot,\cdot)\right),$ where $beta\in\mathbb{R}^p$ is unknown, and that our observations $Y(x)|f \sim N(f(x),\sigma^2).$ Note this assumptions implies the independence of $\left\lbrace Y(x)|x\in\mathcal{X}\right\rbrace$ given $f.$ Then, we can define the error process

<p>
  $$\epsilon(x) := Y(x) - m^\top(x)\beta,$$
</p>
...
Furthermore, our observations can be induced:

<p>
  $$\mathbf{Y} := \left(Y(x_1),\cdots,Y(x_n)\right)^\top\sim N\left(\mathbf{M}\beta,\Sigma\right),\ \ \mathbf{M} = \left(m(x_1),\cdots,m(x_n)\right)^\top\in\mathbb{R}^{n\times p}.$$
</p>
