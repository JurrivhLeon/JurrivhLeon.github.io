...
**Theorem 4** (Universal kriging). Suppose the covariance matrix of $\mathbf{Y}=\left(Y(x_1),\cdots,Y(x_n)\right)$ is nonsingular, and $\mathbf{M}=\left(\mathbf{m}(x_1),\cdots,\mathbf{m}(x_n)\right)^\top$ is of full column rank. Then the BLUP of $Y(x)$ is given by
<p>
  $$\hat{Y}(x) = \mathbf{k}(x)^\top\Sigma^{-1}\left(\mathbf{Y} - \mathbf{M}\hat{\beta}\right) - \mathbf{m}(x)^\top\hat{\beta},$$
</p>
