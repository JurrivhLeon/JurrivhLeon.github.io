...
**Properties.** Suppose $\lbrace \psi_i\rbrace_{i\in\mathbb{N}^{\*}}$ is an orthogonal basis over $L^2_ \mu(\mathcal{X}).$ For any $f\in L^2_\mu(\mathcal{X}),$ it has the representation $f=\sum_{n\in\mathbb{N}^{\*}} c_n\psi_n.$
...
+ (Legendre Polynomials). Consider function approximations on the closed interval $[-1,1],$
