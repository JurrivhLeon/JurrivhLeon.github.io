...
However, the term $p^{* }(z\vert\mathrm{do}(x))$ is neither transportable from $\pi_b$ since $Z$ is directly connected to a selection node, nor equivalent to $p^{* }(z|x)$ as in equation (6) due to the unobserved confounding $U_0$ between $X$ and $Z.$ Hence, transportability is impossible in both diagrams.
