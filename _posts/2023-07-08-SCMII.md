...
<p>
   ...
   &\quad(\text{by Theorem 4 \& do-calculus rule 3 that}\ Z\perp X\vert S\ \text{in}\ \mathcal{G}_{\overline{X(S)}})\\
...
**Remark.** Theorem 5 is proven to be a necessary and sufficient condition for transporting causal effect estimates across domains.
...
<p>
   ...
   p^{* }(y\vert\mathrm{do}(x)) &= p(y\vert\mathrm{do}(x),s) \\
   &= \sum_z p(y\vert\mathrm{do}(x), z, s)p(z|\mathrm{do}(x), s)\\
   &\quad(\text{by Theorem 4 \& do-calculus rule 2 that}\ Z\perp X\vert S\ \text{in}\ \mathcal{G}_{\underline{X}})\\
