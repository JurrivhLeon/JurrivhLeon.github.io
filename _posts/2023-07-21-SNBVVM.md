...
Hence $\lbrace M_ t^\lambda\rbrace_ {t=0}^\infty$ is a supermartingale with $0\leq\mathbb{E}[M_ t^\lambda|\mathcal{F}_ {t-1}]\leq 1.$
...
**Proof.** By definition, $M_ t^\lambda$ can be represented as
<p>$$M_t^\lambda = \exp\left(\frac{\lambda^\top S_t}{\sigma} - \frac{1}{2}\Vert\lambda\Vert_{V_t-V_0}^2\right).$$</p>
...
where $\mathcal{F}_ \infty = \bigcup_ {t=0}^\infty\mathcal{F}_ t.$ Clearly, $\mathbb{E}[M_ t] = \mathbb{E}[\mathbb{E}[M_ t^\Lambda|\Lambda]] \leq 1.$
...
<p>$$\begin{align*}
  M_t &= \int \exp\left(\frac{\lambda^\top S_t}{\sigma} - \frac{1}{2}\Vert\lambda\Vert_{V_t-V_0}^2\right)\phi(\lambda;0,V_0^{-1})\mathrm{d}\lambda\\
  &= \int \exp\left(-\frac{1}{2}\left\Vert\lambda - \sigma^{-1}V_t^{-1}S_t\right\Vert_{V_t-V_0}^2 + \frac{1}{2}\Vert \sigma^{-1}S_t\Vert_{(V_t-V_0)^{-1}}^2\right)\phi(\lambda;0,V_0^{-1})\mathrm{d}\lambda\\
  &= \frac{1}{c(V_0)}\exp\left(\frac{1}{2}\Vert \sigma^{-1}S_t\Vert_{(V_t - V_0)^{-1}}^2\right)\int\exp\left(-\frac{1}{2}\left\lbrace\left\Vert\lambda - \sigma^{-1}V_t^{-1}S_t\right\Vert_{V_t}^2 + \Vert\lambda\Vert_{V_0}^2\right\rbrace\right)\mathrm{d}\lambda.
\end{align*}$$</p>

It can be verified that
<p>$$\begin{align*}
  &\left\Vert\lambda - \sigma^{-1}V_t^{-1}S_t\right\Vert_{V_t-V_0}^2 + \Vert\lambda\Vert_{V_0}^2 = \lambda^\top(V_0+V_t)\lambda - 2\sigma^{-1}\lambda^\top S_t + \sigma^{-2}S_t^\top V_t^{-1} S_t\\
  &= \Vert \lambda - \sigma^{-1}(V_0+V_t)^{-1}S_t\Vert_{V_t}^2 + \Vert \sigma^{-1}S_t\Vert_{V_t^{-1}}^2 - \Vert \sigma^{-1}S_t\Vert_{(V_0+V_t)^{-1}}^2,
  \end{align*}$$</p>

then
<p>$$\begin{align*}
  M_t &= \frac{1}{c(V_0)}\exp\left(\frac{1}{2}\Vert \sigma^{-1}S_t\Vert_{(V_0+V_t)^{-1}}^2\right)\int\exp\left(-\frac{1}{2}\Vert \lambda - \sigma^{-1}(V_0+V_t)^{-1}S_t\Vert_{V_t}^2\right)\mathrm{d}\lambda\\
  &= \frac{c(V_t)}{c(V_0)}\exp\left(\frac{1}{2}\Vert \sigma^{-1}S_t\Vert_{(V_0+V_t)^{-1}}^2\right)\\
  &= \sqrt{\frac{\det(V_0)}{\det(V_0+V_t)}}\exp\left(\frac{1}{2}\Vert \sigma^{-1}S_t\Vert_{(V_0+V_t)^{-1}}^2\right).
\end{align*}$$</p>
