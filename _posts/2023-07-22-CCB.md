...
**Remark.** A solution $h=(h_1, g)$ to the IES in Theorem 1 exists if and only if there exists a solution $h_1$ to the following equation:
...
1. (PV completeness). For any $a\in\mathcal{A},x\in\mathcal{X},$ $\mathbb{E}_ \mathrm{obs}[\sigma(U)|X=x,A=a,Z=z,R_ Z=1]$ holds for any $z\in\mathcal{Z}$ if and only if $\sigma(U)=0$ almost surely.
