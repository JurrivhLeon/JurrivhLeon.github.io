...
## Overall Complexity
With $l$ smaller than $g,$ the overall computational complexity of ```TwinGP``` is $O(g^3 + tg^2l)$ where $t$ is the number of testing locations. If we select $g$ to be order of $\sqrt{n},$ then the training complexity reduces to $O(n^{3/2}).$
