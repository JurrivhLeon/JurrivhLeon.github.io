...
We consider the task of minimizing some black-box function $f:\mathbb{R}^d\to\mathbb{R}$ which could be expensive to evaluate. Suppose we have a noise-free dataset $\mathcal{D}_ n = \lbrace (x_i,f(x_i))\rbrace_ {i=1}^n.$ We use a Gaussian process surrogate model to approximate $f.$
